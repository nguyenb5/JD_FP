 ...
 */
void readValues(){//returns the state of the circuit and how the battery should be charged. 
   ...
   int currentLoadTemporary       = analogRead(currentLoadPin); // Iload
    ...
    // Update Global Vars
    currentFromPanel  = -((currentPanelTemporary*5.0/1024)-2.55)/0.165;
    currentToSystem   = -((currentOutTemporary*5.0/1024)-2.55)/0.165;
    currentToLoad     = -((currentLoadTemporary*5.0/1024)-2.55)/0.165;
...
void sendChargingData(){
  ...
  Serial.println(voltgeFromPanel*currentFromPanel); // power from panel
}
